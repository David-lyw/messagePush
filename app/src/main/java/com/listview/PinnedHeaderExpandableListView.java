...
/**
 * Created by sd on 2015/10/27.
 * <p/>
 * 本控件使用方法：
 * 前提：数据必须提前准备好。
 * 例子1：使用字符串数组。在initData()中初始化好数据（groupData[]，childData[][])
 * 例子2：显示从网络获取的数据（任何数据类型，对象等复杂数据类型）
 * 方法：1：从:上一activity获取好数据（必须）。
 * 2：从上一activity将数据传递到要显示的activity.
 * 3:在显示数据之前整理数据（比如在initData()中整理）。
 * <p/>
 * 适配器显示的例子是显示字符串。可将传递的数据改成List,Map等数据集合。
 */
...
public class PinnedHeaderExpandableListView extends ExpandableListView implements OnScrollListener, OnGroupClickListener {
    ...
    /**
     * 用于在列表头悬浮显示的 View, mHeaderViewVisible 为 true 才可见
     */
